  ...
  <v-container fluid class="ma-0 pa-0">
    ...
    <v-row class="ma-0 pa-0" v-else>
      <v-col md="12">
        ...
        >
          <v-toolbar-title class="google-font mr-3"
            >Manage Keys & Securities</v-toolbar-title
          >
          <v-spacer></v-spacer>
          <v-btn
            depressed
      ...
      <v-col cols="12" md="6" class="mb-0 pb-0">
        <v-text-field
          class="my-0 py-0"
          label="Cloud Messaging Server Key"
          v-model="key.server_key"
          outlined
        ></v-text-field>
      </v-col>
      <v-col cols="12" md="6" class="mb-0 pb-0">
        <v-text-field
          class="my-0 py-0"
          label="Cloud Messaging Web Push Certificates"
          v-model="key.web_push_certificate"
          outlined
        ></v-text-field>
      </v-col>
    </v-row>
  ...
  methods: {
    ...
    setData() {
      ...
      firebase.firestore
        ...
        .set(this.key)
        .then(() => {
          localStorage.setItem("keysandsecurity", JSON.stringify(this.key));
