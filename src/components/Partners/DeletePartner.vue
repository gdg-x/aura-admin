  ...
  <v-dialog v-model="dialog" max-width="500">
    ...
    <v-card v-if="dialog">
      ...
      <v-card-text class="google-font"
        >Would you like to remove {{ PartnerInfo.name }} form the
        Partners?</v-card-text
      >
      <v-card-actions>
        ...
        <v-btn
          ...
          text
          :loading="loading"
          @click="deleteItem(PartnerInfo.id)"
          >Agree</v-btn
        >
      </v-card-actions>
    </v-card>
...
export default {
  ...
  methods: {
    deleteItem(id) {
      this.loading = true;
      PartnersServices.removePartner(this.PartnerInfo.id)
        .then((res) => {
          if (res.success == true) {
            this.loading = false;
            this.dialog = false;
